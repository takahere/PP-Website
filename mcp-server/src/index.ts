...
async function fetchFromAPI(endpoint: string, params?: Record<string, string>): Promise<unknown> {
  ...
  try {
    ...
    switch (name) {
      case 'fetch_analytics_data':
        result = await handleFetchAnalyticsData(args as Parameters<typeof handleFetchAnalyticsData>[0])
        break
      case 'generate_hypothesis':
        result = await handleGenerateHypothesis(args as Parameters<typeof handleGenerateHypothesis>[0])
        break
      case 'ask_clarifying_questions':
        result = await handleAskClarifyingQuestions(args as Parameters<typeof handleAskClarifyingQuestions>[0])
        break
      case 'generate_sheet_data':
        result = await handleGenerateSheetData(args as Parameters<typeof handleGenerateSheetData>[0])
        break
      default:
